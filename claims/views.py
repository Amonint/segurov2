...
from accounts.decorators import permission_required as role_permission_required
from accounts.decorators import requester_own_data_only
...
def claim_list(request):
    ...
    claims = (
        Claim.objects.all()
        .select_related("policy__insurance_company", "reported_by", "assigned_to")
        .order_by("-created_at")
    )
    ...
    for claim in claims.filter(status__in=["docs_pendientes", "enviado_aseguradora"]):
        ...
        if alerts:
            claims_with_alerts.append({"claim": claim, "alerts": alerts})
...
def my_claims(request):
    ...
    claims = (
        Claim.objects.filter(reported_by=request.user)
        .select_related("policy__insurance_company", "asset", "assigned_to")
...
def claim_detail(request, pk):
    ...
    if request.user.role in ["insurance_manager", "admin"]:
        ...
        for doc_type, display_name in required_docs:
            ...
            doc_obj = next(
                (doc for doc in documents if doc.document_type == doc_type), None
            )
            ...
            validation_checklist.append(
                {
                    ...
                    "status_text": (
                        _("Cargado")
                        if is_present
                        else (_("Vencido") if is_overdue else _("Pendiente"))
                    ),
                }
            )
    ...
    if claim.status in ["docs_pendientes", "rechazado"]:
        last_change = (
            claim.timeline.filter(event_type="status_change")
            .order_by("-created_at")
            .first()
        )
        ...
        if claim.status == "reportado":
            management_actions.extend(
                [
                    {
                        "action": "request_docs",
                        "label": _("Solicitar Documentos"),
                        "style": "btn-warning",
                    },
                    {
                        "action": "approve_direct",
                        "label": _("Aprobar Directamente"),
                        "style": "btn-success",
                    },
                ...
                [
                    {
                        ...
                        "label": _("Marcar Documentos Completos"),
                        "style": "btn-info",
                    },
                    {
                        "action": "request_more_docs",
                        "label": _("Solicitar Más Documentos"),
                        "style": "btn-warning",
                    },
                    {"action": "reject", "label": _("Rechazar"), "style": "btn-danger"},
                ]
            ...
            management_actions.extend(
                [
                    {
                        "action": "send_to_insurer",
                        "label": _("Enviar a Aseguradora"),
                        "style": "btn-primary",
                    },
                    {"action": "reject", "label": _("Rechazar"), "style": "btn-danger"},
                ]
            ...
            management_actions.extend(
                [
                    {
                        "action": "under_evaluation",
                        "label": _("Enviar a Revisión"),
                        "style": "btn-info",
                    },
                    {"action": "reject", "label": _("Rechazar"), "style": "btn-danger"},
                ]
    ...
    if (
        request.user.role in ["insurance_manager", "admin"]
        and claim.status == "en_revision"
    ):
        required_docs = ClaimDocument.get_required_documents_for_claim(claim)
        uploaded_types = [doc.document_type for doc in documents]
    ...
    context = {
        ...
        "can_edit": request.user.has_role_permission("claims_write")
        or claim.reported_by == request.user,
...
def claim_create(request):
    ...
    messages.info(
        request,
        _(
            "Para reportar un siniestro, seleccione el bien afectado de su lista de bienes"
        ),
    )
...
def claim_edit(request, pk):
    ...
    if claim.status in ["paid", "closed", "rejected"]:
        messages.error(
            request, _("No se pueden editar siniestros que ya han sido finalizados.")
        )
        return redirect("claims:claim_detail", pk=pk)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            old_values = {
                ...
                "assigned_to": (
                    claim.assigned_to.username if claim.assigned_to else None
                ),
            }
            ...
            new_values = {
                "policy": (
                    updated_claim.policy.policy_number if updated_claim.policy else None
                ),
                ...
                "assigned_to": (
                    updated_claim.assigned_to.username
                    if updated_claim.assigned_to
                    else None
                ),
            }
            ...
            if claim.status == "requiere_cambios":
                ...
                ClaimTimeline.objects.create(
                    ...
                    event_description=_(
                        "Siniestro reenviado para validación por el custodio"
                    ),
                ...
                messages.info(
                    request, _("El siniestro ha sido reenviado para validación.")
                )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.success(
                    request, _("Estado del siniestro actualizado exitosamente.")
                )
                ...
                if new_status == "liquidado" and not hasattr(claim, "settlement"):
                    try:
                        ...
                        if claim.cobertura_aplicada:
                            deductible = claim.cobertura_aplicada.calcular_deducible(
                        ...
                        messages.success(
                            request, _("Finiquito generado automáticamente.")
                        )
                        return redirect(
                            "claims:claim_settlement_detail", pk=settlement.pk
                        )
                    except Exception as e:
                        messages.error(
                            request, f"Error generando finiquito automático: {str(e)}"
                        )
                        # Fallback to create view if auto fails
                        return redirect(
                            "claims:claim_settlement_create", claim_pk=claim.pk
                        )
        ...
        form = ClaimStatusChangeForm(
            claim=claim, user=request.user, initial=initial_data
        )

    return render(
        request,
        "claims/claim_change_status.html",
        {
            "form": form,
            "claim": claim,
        },
...
def claim_approve(request, pk):
    ...
    if claim.status not in ["pendiente", "en_revision"]:
        messages.error(
            request,
            _("Solo se pueden aprobar siniestros en estado pendiente o en revisión."),
        )
        return redirect("claims:claim_detail", pk=pk)
    ...
    try:
        ...
        try:
            ...
            messages.success(
                request, _("Siniestro aprobado y finiquito generado automáticamente.")
            )
            return redirect("claims:claim_settlement_detail", pk=settlement.pk)

        except Exception as e:
            messages.warning(
                request, f"Siniestro aprobado, pero error generando finiquito: {str(e)}"
            )
...
def claim_request_changes(request, pk):
    ...
    if not comments:
        messages.error(
            request, _("Debe proporcionar comentarios sobre los cambios requeridos.")
        )
        return redirect("claims:claim_detail", pk=pk)
...
def claim_delete(request, pk):
    ...
    if claim.status not in ["reported", "documentation_pending"]:
        messages.error(
            request, _("Solo se pueden eliminar siniestros en estado inicial.")
        )
        return redirect("claims:claim_detail", pk=pk)
    ...
    if claim.documents.exists() or ClaimSettlement.objects.filter(claim=claim).exists():
        messages.error(
            request,
            _(
                "No se puede eliminar el siniestro porque tiene documentos o finiquitos asociados."
            ),
        )
        return redirect("claims:claim_detail", pk=pk)
...
def claim_documents(request, pk):
    ...
    return render(
        request, "claims/claim_documents.html", {"claim": claim, "documents": documents}
    )
...
def claim_document_upload(request, pk):
    ...
    if request.user.role == "requester" and claim.reported_by != request.user:
        messages.error(
            request, _("No tienes permisos para subir documentos a este siniestro.")
        )
        return redirect("claims:my_claims")
    ...
    if request.method == "POST":
        ...
        if not document_name or not document_type or not file:
            messages.error(
                request, _("Por favor complete todos los campos requeridos.")
            )
        ...
        try:
            ...
            messages.error(
                request, _("Error al subir el documento: %(error)s") % {"error": str(e)}
            )
            return redirect("claims:claim_documents", pk=pk)

    return render(
        request,
        "claims/claim_document_upload.html",
        {"claim": claim, "document_types": ClaimDocument.DOCUMENT_TYPE_CHOICES},
    )
...
def claim_document_delete(request, pk):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            _('Documento "%(name)s" eliminado exitosamente.') % {"name": document_name},
        )
        return redirect("claims:claim_documents", pk=claim_pk)

    return render(
        request,
        "claims/claim_document_delete_confirm.html",
        {"document": document, "claim": document.claim},
    )
...
def claim_settlements_list(request):
    ...
    return render(
        request,
        "claims/claim_settlements_list.html",
        {
...
def claim_settlement_create(request, claim_pk):
    ...
    return render(
        request,
        "claims/claim_settlement_form.html",
        {
            "form": form,
            "claim": claim,
...
def claim_settlement_detail(request, pk):
    ...
    return render(
        request,
        "claims/claim_settlement_detail.html",
        {
            "settlement": settlement,
...
def claim_settlement_approve(request, pk):
    ...
    if settlement.status != "pending_approval":
        messages.error(
            request, _('El finiquito debe estar en estado "Pendiente de Aprobación".')
        )
        return redirect("claims:claim_settlement_detail", pk=pk)
...
def claim_settlement_sign(request, pk):
    ...
    if settlement.status != "approved":
        messages.error(
            request, _("El finiquito debe estar aprobado antes de firmarse.")
        )
        return redirect("claims:claim_settlement_detail", pk=pk)
...
def claim_settlement_pay(request, pk):
    ...
    if not request.user.get_role_permissions().__contains__("claims_write"):
        messages.error(
            request, _("No tienes permisos para marcar finiquitos como pagados.")
        )
        return redirect("claims:claim_settlement_detail", pk=pk)

    if settlement.status != "signed":
        messages.error(
            request, _("El finiquito debe estar firmado antes de marcarlo como pagado.")
        )
        return redirect("claims:claim_settlement_detail", pk=pk)
...
def claim_request_documents(request, pk):
    ...
    if request.method == "POST":
        ...
        created_docs = ClaimDocument.create_required_documents_for_claim(
            claim, request.user
        )
        ...
        if created_docs:
            ...
            messages.success(
                request,
                _("Se han solicitado %(count)d documentos requeridos.")
            ...
            messages.info(
                request, _("Todos los documentos requeridos ya han sido solicitados.")
            )
    ...
    return render(
        request,
        "claims/claim_request_documents.html",
        {
            "claim": claim,
