...
class Claim(models.Model):
    ...
    claim_number = models.CharField(
        _("Número de siniestro"), max_length=50, unique=True
    )
    ...
    fecha_siniestro = models.DateField(
        _("Fecha del siniestro"), help_text=_("Fecha real en que ocurrió el siniestro")
    )
    ...
    causa = models.TextField(
        _("Causa del siniestro"), help_text=_("Descripción detallada de la causa")
    )
    ...
    incident_description = models.TextField(
        _("Descripción del incidente"), help_text=_("Descripción general del incidente")
    )
    ...
    asset_type = models.CharField(_("Tipo de bien afectado"), max_length=100)
    asset_description = models.CharField(_("Descripción del bien"), max_length=255)
    asset_code = models.CharField(_("Código del bien"), max_length=50, blank=True)
    ...
    status = models.CharField(
        _("Estado"), max_length=25, choices=STATUS_CHOICES, default="pendiente"
    )
    ...
    def clean(self):
        ...
        if (
            ...
            and self.report_date < self.incident_date
        ):
            raise ValidationError(
                _("La fecha de reporte no puede ser anterior a la fecha del incidente")
            )

        # Validate approved amount
        if self.approved_amount and self.approved_amount > self.estimated_loss:
            raise ValidationError(
                _("El monto aprobado no puede ser mayor a la pérdida estimada")
            )
    ...
    def change_status(self, new_status, user, notes=None):
        ...
        if not self.can_change_status(new_status, user):
            raise ValidationError(
                _("No tiene permisos para cambiar el estado a %(status)s")
                % {"status": new_status}
            )
        ...
        if new_status in ["en_revision", "liquidado", "rechazado", "pagado"]:
            manager_admin_users = UserProfile.objects.filter(
                role__in=["insurance_manager", "admin"], is_active=True
            ).exclude(
                id=user.id
            ...
            for manager_user in manager_admin_users:
                Notification.create_notification(
                    ...
                    message=status_messages.get(
                        new_status,
                        f"Siniestro {self.claim_number} actualizado a {new_status}.",
                    ),
    ...
    def get_days_in_current_status(self):
        ...
        last_timeline = (
            self.timeline.filter(event_type="status_change", new_status=self.status)
            .order_by("-created_at")
            .first()
        )
    ...
    def generate_claim_number():
        ...
        last_claim = (
            Claim.objects.filter(claim_number__startswith=f"SIN-{current_year}-")
        ...
        if self.asset:
            ...
            if not self.incident_location:
                self.incident_location = (
                    self.ubicacion_detallada[:255] if self.ubicacion_detallada else ""
                )
    ...
    def _auto_create_settlement(self, approved_by, approval_notes):
        ...
        try:
            ...
            logging.error(
                f"Error creating automatic settlement for claim {self.claim_number}: {e}"
            )
...
class ClaimDocument(models.Model):
    ...
    document_name = models.CharField(_("Nombre del documento"), max_length=255)
    document_type = models.CharField(
        _("Tipo de documento"), max_length=20, choices=DOCUMENT_TYPE_CHOICES
    )
    ...
    is_latest_version = models.BooleanField(
        _("Es la versión más reciente"), default=True
    )

    # File fields
    file = models.FileField(_("Archivo"), upload_to="claims/documents/%Y/%m/")
    file_size = models.PositiveIntegerField(_("Tamaño del archivo"), editable=False)
    ...
    is_required = models.BooleanField(_("¿Es requerido?"), default=False)
    ...
    # Status and audit
    status = models.CharField(
        _("Estado"), max_length=20, choices=STATUS_CHOICES, default="active"
    )
        ...
        for doc_type, doc_name in required_docs:
            ...
            if not existing:
                ...
                document = ClaimDocument.objects.create(
                    ...
                    is_required=True,
                    required_deadline=(
                        claim.fecha_solicitud_documentos + timedelta(days=30)
                        if claim.fecha_solicitud_documentos
...
class ClaimTimeline(models.Model):
    ...
    old_status = models.CharField(_("Estado anterior"), max_length=25, blank=True)
    new_status = models.CharField(_("Estado nuevo"), max_length=25, blank=True)
    ...
    def __str__(self):
        return (
            f"{self.claim.claim_number} - {self.event_type} ({self.created_at.date()})"
        )
...
class ClaimSettlement(models.Model):
    ...
    settlement_number = models.CharField(
        _("Número de Finiquito"), max_length=50, unique=True, blank=True
    )
    ...
    payment_date = models.DateField(
        _("Fecha de Pago"), null=True, blank=True, help_text=_("Fecha de pago (alias)")
    )
    payment_reference = models.CharField(
        _("Referencia de Pago"), max_length=100, blank=True
    )
    ...
    signed_date = models.DateField(_("Fecha de Firma"), null=True, blank=True)

    # Documents
    settlement_document = models.FileField(
        _("Documento de Finiquito"), upload_to="settlements/", blank=True, null=True
    )
    ...
    def generate_settlement_number(self):
        ...
        current_year = timezone.now().year
        last_settlement = (
            ClaimSettlement.objects.filter(
