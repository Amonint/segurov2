...
from .models import Alert, EmailLog, Notification
...
def execute_policy_expiring_alert(alert):
    ...
    expiring_policies = Policy.objects.filter(
        end_date__lte=expiry_date, end_date__gte=timezone.now().date(), status="active"
        ...
        if policy.responsible_user:
            notification = Notification.create_notification(
                ...
                priority=normalize_priority(
                    "high" if days_until_expiry <= 7 else "normal"
                ),
...
def execute_policy_expired_alert(alert):
    ...
    expired_policies = Policy.objects.filter(
        end_date__gte=week_ago, end_date__lt=timezone.now().date(), status="active"
    ).select_related("responsible_user", "insurance_company")
...
def execute_document_overdue_alert(alert):
    ...
    overdue_documents = (
        ClaimDocument.objects.filter(
            is_required=True, required_deadline__lt=timezone.now().date()
        )
        .exclude(claim__status__in=["closed", "paid", "rejected"])
...
def execute_maintenance_required_alert(alert):
    ...
    maintenance_assets = Asset.objects.filter(
        condition_status__in=["regular", "malo"], is_insured=True
...
def run_scheduled_alerts():
    ...
    for alert in alerts:
        if alert.should_run():
            ...
            total_results["notifications_created"] += results.get(
                "notifications_created", 0
            )
