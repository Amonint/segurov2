...
def email_templates_list(request):
    ...
    return render(request, 'notifications/email_templates_list.html', {
        'templates': templates,
...
def email_templates_create(request):
    ...
    return render(request, 'notifications/email_templates_form.html', {
        'form': form,
...
def email_templates_edit(request, pk):
    """Edit email template"""
    if not request.user.get_role_permissions().__contains__('settings_write'):
        messages.error(request, _('No tienes permisos para editar plantillas de email.'))
        return redirect('notifications:email_templates_list')
...
def email_logs_list(request):
    ...
    return render(request, 'notifications/email_logs_list.html', {
        'logs': logs,
        'stats': stats,
...
def email_logs_detail(request, pk):
    ...
    log = get_object_or_404(
        EmailLog.objects.select_related('template', 'claim', 'policy', 'invoice', 'created_by'),
        pk=pk
    )

    return render(request, 'notifications/email_logs_detail.html', {
        'log': log,
...
def test_email_template(request, pk):
    ...
    if not request.user.get_role_permissions().__contains__('settings_write'):
        messages.error(request, _('No tienes permisos para probar plantillas de email.'))
        return redirect('notifications:email_templates_list')
    ...
    return render(request, 'notifications/email_template_test.html', {
        'template': template,
        'context': context,
...
def alert_create(request):
    ...
    return render(request, 'notifications/alert_form.html', {
        'form': form,
...
def alert_detail(request, pk):
    ...
    return render(request, 'notifications/alert_detail.html', {
        'alert': alert,
    })
...
def alert_execute(request, pk):
    ...
    messages.success(request, f'Alerta ejecutada: {results.get("notifications_created", 0)} notificaciones creadas, {results.get("emails_sent", 0)} emails enviados.')
...
def run_all_alerts(request):
    ...
    messages.success(request, f'Alertas ejecutadas: {results.get("alerts_executed", 0)} alertas procesadas, {results.get("notifications_created", 0)} notificaciones creadas, {results.get("emails_sent", 0)} emails enviados.')
