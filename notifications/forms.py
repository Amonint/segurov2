...
from .models import EmailTemplate, Alert
...
class EmailTemplateForm(forms.ModelForm):
    ...
    class Meta:
        model = EmailTemplate
        fields = [
            'name', 'template_type', 'recipient_type',
    ...
    def clean_body_html(self):
        ...
        if body_html and len(body_html.strip()) < 10:
            raise forms.ValidationError(_('El cuerpo HTML debe tener al menos 10 caracteres.'))
        return body_html
    ...
    def clean_subject(self):
        ...
        if subject and '{{' in subject and '}}' not in subject:
            raise forms.ValidationError(_('Las variables en el asunto deben estar completas (ej: {{variable}}).'))
        return subject
...
class AlertForm(forms.ModelForm):
    ...
    class Meta:
        model = Alert
        fields = [
            'name', 'alert_type', 'description', 'conditions',
            'recipients', 'email_recipients', 'frequency', 'is_active'
        ]
        widgets = {
            'description': forms.Textarea(attrs={'rows': 3}),
            'conditions': forms.Textarea(attrs={'rows': 5, 'placeholder': 'Ejemplo: {"days_ahead": 30}'}),
            'email_recipients': forms.Textarea(attrs={'rows': 2, 'placeholder': 'email1@example.com, email2@example.com'}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Help texts
        self.fields['conditions'].help_text = _('Condiciones específicas en formato JSON. Ejemplo: {"days_ahead": 30} para alertas de vencimiento.')
        self.fields['email_recipients'].help_text = _('Direcciones de email adicionales separadas por comas.')
    ...
    def clean_conditions(self):
        ...
        if isinstance(conditions, str):
            try:
                ...
                raise forms.ValidationError(_('Las condiciones deben ser un JSON válido.'))
        return conditions
