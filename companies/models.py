...
class EmissionRightsManager(models.Manager):
    ...
    def get_emission_right(self, premium_amount):
        ...
        try:
            right = self.filter(
                min_amount__lte=premium_amount,
                max_amount__gte=premium_amount
            ).first()
...
class EmissionRights(models.Model):
    ...
    is_active = models.BooleanField(
        _('Activo'),
        default=True,
        help_text=_('Si está activo para cálculos')
    )
    ...
    def clean(self):
        ...
        if self.is_active:
            ...
            for existing in overlapping:
                if (self.min_amount <= existing.max_amount and self.max_amount >= existing.min_amount):
                    raise ValidationError(_('Los rangos de montos no pueden solaparse con configuraciones activas.'))
    ...
    def is_currently_valid(self):
        ...
        return (self.valid_from <= today and
                (self.valid_until is None or self.valid_until >= today) and
                self.is_active)
...
class PolicyRetention(models.Model):
    ...
    def clean(self):
        ...
        if self.custom_percentage and (self.custom_percentage < 0 or self.custom_percentage > 100):
            raise ValidationError(_('El porcentaje personalizado debe estar entre 0 y 100.'))