...
from .models import InsuranceCompany, EmissionRights, RetentionType, PolicyRetention
from .forms import (
    InsuranceCompanyForm, InsuranceCompanySearchForm,
    EmissionRightsForm, RetentionTypeForm, PolicyRetentionForm
)
...
def insurance_company_list(request):
    ...
    companies_with_policies = companies.filter(policies__isnull=False).distinct().count()
...
def insurance_company_create(request):
    ...
    return render(request, 'companies/insurance_company_form.html', {
        'form': form,
...
def insurance_company_detail(request, pk):
    ...
    company = get_object_or_404(
        InsuranceCompany.objects.prefetch_related('policies'),
        pk=pk
    )
...
def insurance_company_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, _('Compañía aseguradora actualizada exitosamente.'))
...
def insurance_company_delete(request, pk):
    ...
    if company.policies.exists():
        messages.error(request, _('No se puede eliminar la compañía porque tiene pólizas asociadas.'))
...
def insurance_company_toggle_active(request, pk):
    ...
    messages.success(request, _('Compañía %(action)s exitosamente.') % {'action': action})
    return redirect('companies:insurance_company_detail', pk=pk)
...
def emission_rights_list(request):
    ...
    if not request.user.get_role_permissions().__contains__('insurance_companies_read'):
        messages.error(request, _('No tienes permisos para ver la configuración financiera.'))
...
def emission_rights_create(request):
    ...
    if not request.user.get_role_permissions().__contains__('insurance_companies_write'):
        messages.error(request, _('No tienes permisos para crear configuración financiera.'))
    ...
    return render(request, 'companies/emission_rights_form.html', {
        'form': form,
...
def emission_rights_edit(request, pk):
    ...
    if not request.user.get_role_permissions().__contains__('insurance_companies_write'):
        messages.error(request, _('No tienes permisos para editar configuración financiera.'))
        return redirect('companies:emission_rights_list')
...
def emission_rights_delete(request, pk):
    ...
    if not request.user.get_role_permissions().__contains__('insurance_companies_delete'):
        messages.error(request, _('No tienes permisos para eliminar configuración financiera.'))
        return redirect('companies:emission_rights_list')
    ...
    return render(request, 'companies/emission_rights_confirm_delete.html', {
        'emission_right': emission_right,
...
def retention_types_list(request):
    ...
    return render(request, 'companies/retention_types_list.html', {
...
def retention_types_create(request):
    ...
    return render(request, 'companies/retention_types_form.html', {
        'form': form,
...
def policy_retentions_list(request):
    ...
    return render(request, 'companies/policy_retentions_list.html', {
...
def policy_retentions_create(request):
    ...
    if not request.user.get_role_permissions().__contains__('insurance_companies_write'):
        messages.error(request, _('No tienes permisos para crear retenciones de póliza.'))
    ...
    return render(request, 'companies/policy_retentions_form.html', {
        'form': form,
...
def policy_retentions_edit(request, pk):
    ...
    if not request.user.get_role_permissions().__contains__('insurance_companies_write'):
        messages.error(request, _('No tienes permisos para editar retenciones de póliza.'))
        return redirect('companies:policy_retentions_list')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, _('Retención de póliza actualizada exitosamente.'))
            return redirect('companies:policy_retentions_list')
    else:
