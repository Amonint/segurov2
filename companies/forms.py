...
class EmissionRightsForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            'min_amount', 'max_amount', 'emission_right',
    ...
    def clean(self):
        ...
        if min_amount and max_amount and min_amount >= max_amount:
            raise forms.ValidationError(_('El monto mínimo debe ser menor al monto máximo'))
...
class RetentionTypeForm(forms.ModelForm):
    ...
    class Meta:
        model = RetentionType
        fields = ['name', 'code', 'retention_type', 'percentage', 'is_active', 'description']
        widgets = {
...
class PolicyRetentionForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            'policy', 'retention_type', 'applies_to_premium',
            'applies_to_total', 'custom_percentage', 'is_active'
        ]
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['custom_percentage'].help_text = _('Opcional. Si no se especifica, se usa el porcentaje estándar del tipo de retención.')
        self.fields['is_active'].label = _('Activo')
    ...
    def clean(self):
        ...
        if not applies_to_premium and not applies_to_total:
            raise forms.ValidationError(_('La retención debe aplicar al menos a la prima o al total'))

        # Validate custom percentage
        if custom_percentage is not None and (custom_percentage < 0 or custom_percentage > 100):
            raise forms.ValidationError(_('El porcentaje personalizado debe estar entre 0 y 100'))
