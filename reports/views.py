...
def report_detail(request, pk):
    ...
    # Check permissions
    if not request.user.has_role_permission('reports_read') and report.created_by != request.user:
    ...
    context = {
        ...
        'can_edit': request.user.has_role_permission('reports_write') or report.created_by == request.user,
        'can_execute': request.user.has_role_permission('reports_execute') or report.created_by == request.user,
    }
...
def report_edit(request, pk):
    ...
    # Check ownership
    if not request.user.has_role_permission('reports_write') and report.created_by != request.user:
...
def report_delete(request, pk):
    ...
    # Check ownership
    if not request.user.has_role_permission('reports_delete') and report.created_by != request.user:
...
def report_execute(request, pk):
    ...
    # Check permissions
    if not request.user.has_role_permission('reports_execute') and report.created_by != request.user:
    ...
    return render(request, 'reports/report_execute.html', {
        'form': form,
        'report': report,
    })
...
def quick_report(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                if export_format == 'html':
                    return render(request, 'reports/quick_report_view.html', {
                        'report_type': report_type,
                        'report_data': report_data,
    ...
    return render(request, 'reports/quick_report.html', {
        'form': form,
    })
