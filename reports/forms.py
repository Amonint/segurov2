        ...
        if is_scheduled:
            if not schedule_frequency:
                raise forms.ValidationError(
                    _("Debe especificar la frecuencia para reportes programados")
                )

            if not recipients:
                raise forms.ValidationError(
                    _(
                        "Debe seleccionar al menos un destinatario para reportes programados"
                    )
                )
...
class QuickReportForm(forms.Form):
    ...
    report_type = forms.ChoiceField(
        choices=Report.REPORT_TYPES, label=_("Tipo de Reporte"), required=True
    )
    export_format = forms.ChoiceField(
        choices=Report.EXPORT_FORMATS, label=_("Formato de Exportación"), initial="html"
    )
    ...
    def clean(self):
        ...
        if not date_range and not (custom_date_from and custom_date_to):
            raise forms.ValidationError(
                _(
                    "Debe seleccionar un período predefinido o especificar fechas personalizadas"
                )
            )
