...
class Report(models.Model):
    ...
    report_type = models.CharField(_('Tipo de Reporte'), max_length=50, choices=REPORT_TYPES)
    ...
    def get_report_data(self):
        ...
        if self.report_type == 'policies_summary':
            ...
            total_insured_value = Policy.objects.filter(status='active').aggregate(
            ...
            data['data'] = {
                'policies': list(expiring_policies.values(
                    'policy_number', 'insurance_company__name',
                    'end_date', 'insured_value', 'responsible_user__username'
                )),
            ...
            open_claims = Claim.objects.filter(status__in=['reported', 'documentation_pending', 'sent_to_insurer', 'under_evaluation']).count()
            closed_claims = Claim.objects.filter(status__in=['closed', 'paid', 'rejected']).count()
            paid_claims = Claim.objects.filter(status='paid').count()

            claims_by_status = Claim.objects.values('status').annotate(count=Count('id'))
            claims_by_cause = Claim.objects.values('incident_description').annotate(count=Count('id')).order_by('-count')[:10]

            total_paid_amount = Claim.objects.filter(status='paid').aggregate(
            ...
            pending_amount = Invoice.objects.filter(payment_status='pending').aggregate(
                total=Sum('total_amount')
            )['total'] or 0
            ...
            avg_resolution_time = Claim.objects.filter(
            ...
            if Claim.objects.count() > 0:
                resolved_claims = Claim.objects.filter(status__in=['closed', 'paid']).count()
            ...
            data['data'] = {
                ...
                'total_open_claims': Claim.objects.filter(status__in=['reported', 'documentation_pending', 'sent_to_insurer', 'under_evaluation']).count(),
            }
