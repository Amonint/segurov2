    ...
    if request.method == "POST":
        ...
        if username and password:
            ...
            if user is not None and user.is_active:
                ...
                messages.success(
                    request, _("Bienvenido, %(name)s!") % {"name": user.get_full_name()}
                )
                return redirect(next_url)
            else:
                messages.error(
                    request, _("Credenciales inválidas. Por favor, inténtelo de nuevo.")
                )
        else:
...
def get_manager_dashboard_data(user):
    ...
    return {
        ...
        "pending_claims": Claim.objects.filter(
            status__in=["reported", "documentation_pending"]
        ).count(),
        "claims_under_evaluation": Claim.objects.filter(
...
def user_toggle_active(request, pk):
    ...
    messages.success(
        request, _("Usuario %(action)s exitosamente.") % {"action": action}
    )
    return redirect("accounts:user_detail", pk=pk)
...
def user_delete(request, pk):
    ...
    has_claims = Claim.objects.filter(
    ...
    if has_policies or has_claims or has_invoices:
        messages.error(
            request,
            _("No se puede eliminar el usuario porque tiene registros asociados."),
        )
        return redirect("accounts:user_detail", pk=pk)
...
def user_change_password(request, pk):
    ...
    if request.user != user and not request.user.has_role_permission("users_manage"):
        messages.error(
            request, _("No tienes permisos para cambiar la contraseña de este usuario.")
        )
        return redirect("accounts:user_detail", pk=pk)
    ...
    if request.method == "POST":
        if request.user == user:
            ...
            if form.is_valid():
                ...
                messages.success(
                    request, _("Tu contraseña ha sido cambiada exitosamente.")
                )
                return redirect("accounts:profile")
        else:
    ...
    return render(
        request,
        "accounts/user_change_password.html",
        {
            "form": form,
            "user_obj": user,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.success(
                    request, _("Se ha enviado un enlace de reset al email del usuario.")
                )
                return redirect("accounts:user_list")
