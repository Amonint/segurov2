...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.WARNING('[*] Iniciando poblacion de base de datos...'))
    ...
    def clear_data(self):
        ...
        from policies.models import Policy, PolicyDocument, Coverage
    ...
    def create_users(self):
        ...
        for user_data in users_data:
            ...
            existing_user = UserProfile.objects.filter(username=user_data['username']).first()
            if existing_user:
    ...
    def create_policies(self, companies, brokers, users):
        ...
        from policies.models import Policy, Coverage
        ...
        policies_data = [
            {
                ...
                'coverages': [
                    {'nombre': 'Danos propios', 'valor_asegurado': Decimal('100000.00'), 'porcentaje_deducible': Decimal('5.00')},
                    {'nombre': 'Responsabilidad civil', 'valor_asegurado': Decimal('200000.00'), 'deducible': Decimal('500.00')},
                    {'nombre': 'Robo total', 'valor_asegurado': Decimal('100000.00'), 'porcentaje_deducible': Decimal('10.00')},
                ],
            },
            {
                'group_type': 'patrimoniales',
                ...
                'responsible_user': admin_user,
                'coverages': [
                    {'nombre': 'Danos por accidente', 'valor_asegurado': Decimal('500000.00'), 'porcentaje_deducible': Decimal('3.00')},
                    {'nombre': 'Robo', 'valor_asegurado': Decimal('300000.00'), 'deducible': Decimal('200.00')},
                    {'nombre': 'Fallas electricas', 'valor_asegurado': Decimal('200000.00'), 'porcentaje_deducible': Decimal('5.00')},
                ],
            },
            {
                'group_type': 'patrimoniales',
                ...
                'responsible_user': admin_user,
                'coverages': [
                    {'nombre': 'Incendio', 'valor_asegurado': Decimal('3000000.00'), 'porcentaje_deducible': Decimal('2.00')},
                    {'nombre': 'Terremoto', 'valor_asegurado': Decimal('1500000.00'), 'porcentaje_deducible': Decimal('5.00')},
                    {'nombre': 'Inundacion', 'valor_asegurado': Decimal('500000.00'), 'porcentaje_deducible': Decimal('3.00')},
                ],
            },
            {
                'group_type': 'patrimoniales',
                ...
                'responsible_user': admin_user,
                'coverages': [
                    {'nombre': 'Robo con violencia', 'valor_asegurado': Decimal('150000.00'), 'deducible': Decimal('1000.00')},
                    {'nombre': 'Hurto', 'valor_asegurado': Decimal('50000.00'), 'porcentaje_deducible': Decimal('10.00')},
                ],
            },
        ]
        ...
        for policy_data in policies_data:
            ...
            policy.base_imponible = policy.prima + policy.contrib_superintendencia + policy.contrib_seguro_campesino + policy.derecho_emision
            ...
            self.stdout.write(f'   [+] Poliza creada: {policy.policy_number} - {policy.subgroup}')
    ...
    def create_assets(self, policies, users):
        ...
        vehicle_policy = next((p for p in policies if 'Vehiculo' in p.subgroup), policies[0])
        electronic_policy = next((p for p in policies if 'Electronico' in p.subgroup), policies[0])
        ...
        for asset_data in assets_data:
            ...
            if created:
                self.stdout.write(f'   [+] Bien creado: {asset.asset_code} - {asset.name} -> Custodio: {asset.custodian.username}')
            else:
    ...
    def create_claims(self, assets, policies, users):
        ...
        if not insured_assets:
            self.stdout.write(self.style.WARNING('   [!] No hay bienes asegurados para crear siniestros'))
            return []
        ...
        claims_data = [
            ...
            {
                'asset': insured_assets[2] if len(insured_assets) > 2 else insured_assets[0],
            ...
            {
                'asset': insured_assets[3] if len(insured_assets) > 3 else insured_assets[0],
        ...
        for claim_data in claims_data:
            ...
            self.stdout.write(f'   [+] Siniestro creado: {claim.claim_number} - {claim.get_status_display()}')
    ...
    def print_summary(self, users, companies, brokers, policies, assets, claims):
        ...
        for user in users:
            ...
            password = 'Admin123!' if user.role == 'admin' else ('Gerente123!' if user.role == 'insurance_manager' else 'Custodio123!')
