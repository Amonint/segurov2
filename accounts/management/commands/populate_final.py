...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("🚀 Iniciando población final de base de datos...")
        )
        ...
        if not all([admin, managers, custodians, companies, brokers]):
            self.stdout.write(
                self.style.ERROR(
                    "❌ Error: Faltan datos base (usuarios, compañías). Ejecuta populate_step1 primero."
                )
            )
            return
        ...
        try:
            with transaction.atomic():
                ...
                Policy.objects.filter(
                    policy_number__startswith="POL-2026-AUTO-"
                ).delete()
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✓ {len(policies)} pólizas creadas exitosamente"
                    )
                )
                ...
                if not policies:
                    self.stdout.write(
                        self.style.ERROR(
                            "⚠ No se crearon pólizas. Deteniendo ejecución."
                        )
                    )
                    return
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✓ {len(updated_assets)} bienes actualizados con póliza"
                    )
                )
                ...
                insured_assets = list(
                    Asset.objects.filter(insurance_policy__isnull=False)
                )
                claims = self.create_claims(insured_assets, managers, admin)
                self.stdout.write(
                    self.style.SUCCESS(f"✓ {len(claims)} siniestros creados")
                )
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"✓ {len(invoices)} facturas creadas")
                )

            self.stdout.write(self.style.SUCCESS("\n" + "=" * 70))
            self.stdout.write(
                self.style.SUCCESS(
                    "✅ ¡Base de datos completa! Todo vinculado correctamente."
                )
            )
    ...
    def assign_policies_to_assets(self, policies):
        ...
        electronic_policy = next(
            (p for p in policies if p.branch == "Equipo Electrónico"),
            policies[0] if policies else None,
        )
        furniture_policy = next(
            (p for p in policies if p.branch == "Incendio y Líneas Aliadas"),
            policies[0] if policies else None,
        )
        ...
        for asset in assets:
            ...
            self.stdout.write(
                f"  ✓ Asignada póliza {target_policy.policy_number} a {asset.name}"
            )
        ...
        for i in range(5):
            try:
                ...
                claim = Claim.objects.create(
                    ...
                    incident_date=today
                    - timedelta(days=random.randint(10, 60)),  # Alias
                ...
                claims.append(claim)
                self.stdout.write(
                    f"  ✓ Siniestro {claim.claim_number} para {asset.name}"
                )
            except Exception as e:
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Error siniestro {i+1}: {str(e)}")
                )
        ...
        for i, policy in enumerate(policies):
            try:
                if Invoice.objects.filter(
                    invoice_number=f"INV-2026-AUTO-{i+1}"
                ).exists():
                    continue
                ...
                invoice = Invoice.objects.create(
                    ...
                    fecha_vencimiento=policy.start_date
                    + timedelta(days=30),  # Alias validation
                    payment_status="pagada" if i == 0 else "pendiente",
                    payment_date=(
                        policy.start_date + timedelta(days=15) if i == 0 else None
                    ),
                    created_by=admin,
                )
                invoices.append(invoice)
                self.stdout.write(
                    f"  ✓ Factura {invoice.invoice_number} para Póliza {policy.policy_number}"
                )
            except Exception as e:
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Error factura {i+1}: {str(e)}")
                )
    ...
    def print_summary(self):
        ...
        self.stdout.write(
            f"  📦 Bienes: {Asset.objects.count()} (Asegurados: {Asset.objects.filter(is_insured=True).count()})"
        )
