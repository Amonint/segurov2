...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("🚀 Poblando pólizas, bienes, siniestros y facturas...")
        )
        ...
        if not all([admin, managers, custodians, companies, brokers]):
            self.stdout.write(
                self.style.ERROR(
                    '❌ Error: Ejecuta primero "python manage.py populate_step1"'
                )
            )
            return
        ...
        try:
            ...
            if not policies:
                self.stdout.write(
                    self.style.WARNING(
                        "⚠ No se pudieron crear pólizas. Creando datos sin pólizas..."
                    )
                )
                return
            ...
            self.stdout.write(
                self.style.SUCCESS("✅ ¡Base de datos completamente poblada!")
            )
    ...
    def create_policies(self, companies, brokers, managers, admin):
        ...
        for i in range(5):
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Error en póliza {i+1}: {str(e)}")
                )
                continue
    ...
    def create_assets(self, policies, custodians):
        ...
        for i in range(15):
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Error en bien {i+1}: {str(e)}")
                )
                continue
    ...
    def create_claims(self, assets, managers, admin):
        ...
        for i in range(10):
            ...
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Error en siniestro {i+1}: {str(e)}")
                )
                continue
    ...
    def create_invoices(self, policies):
        ...
        for i, policy in enumerate(policies[:3]):
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Error en factura {i+1}: {str(e)}")
                )
                continue
