    ...
    def decorator(view_func):
        ...
        def wrapper(request, *args, **kwargs):
            ...
            if not request.user.has_role_permission(permission):
                messages.error(request, _('No tiene permisos suficientes para realizar esta acción.'))
                raise PermissionDenied
...
def owner_or_permission_required(permission, owner_field='user'):
    ...
    def decorator(view_func):
        ...
        def wrapper(request, *args, **kwargs):
            ...
            pk = kwargs.get('pk')
            if not pk:
                raise ValueError("owner_or_permission_required requires a 'pk' parameter in the URL")
            ...
            if pk:
                try:
                    ...
                    if owner != request.user:
                        messages.error(request, _('No tiene permisos para acceder a este recurso.'))
                        raise PermissionDenied
