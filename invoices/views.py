...
from .forms import InvoiceForm, InvoiceSearchForm, InvoicePaymentForm, InvoiceBulkActionForm
...
def invoice_list(request):
    ...
    pending_amount = invoices.filter(payment_status='pending').aggregate(
        total=Sum('total_amount')
    )['total'] or 0
...
def invoice_detail(request, pk):
    ...
    if not request.user.has_role_permission('invoices_read'):
        ...
        if request.user != invoice.created_by and request.user != invoice.policy.responsible_user:
    ...
    context = {
        ...
        'days_overdue': (timezone.now().date() - invoice.due_date).days if invoice.due_date < timezone.now().date() else 0,
    }
...
def invoice_edit(request, pk):
    ...
    if invoice.payment_status == 'paid':
        messages.error(request, _('No se pueden editar facturas que ya han sido pagadas.'))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            AuditLog.log_action(
                ...
                new_values={'payment_status': 'paid', 'payment_date': str(updated_invoice.payment_date)}
            )
...
def invoice_delete(request, pk):
    ...
    if invoice.payment_status == 'paid':
        messages.error(request, _('No se pueden eliminar facturas que ya han sido pagadas.'))
        return redirect('invoices:invoice_detail', pk=pk)
...
def invoice_pdf(request, pk):
    ...
    messages.info(request, _('Funcionalidad de PDF en desarrollo. Por ahora puede imprimir la página.'))
    return redirect('invoices:invoice_detail', pk=pk)
...
def invoice_bulk_action(request):
    ...
    if action == 'mark_paid':
        ...
        if updated_count > 0:
            messages.success(request, f'{updated_count} facturas marcadas como pagadas.')
        else:
            messages.warning(request, _('Todas las facturas seleccionadas ya estaban pagadas.'))
