...
class InvoiceForm(forms.ModelForm):
    ...
    class Meta:
        model = Invoice
        fields = ["policy", "invoice_date", "due_date", "created_by"]
        widgets = {
    ...
    def clean(self):
        ...
        if policy and policy.status not in ["active", "expired"]:
            raise forms.ValidationError(
                _("No se pueden crear facturas para pólizas canceladas o renovadas")
            )
...
class InvoiceSearchForm(forms.Form):
    ...
    search = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"placeholder": _("Número de factura, póliza...")}
        ),
    )
...
class InvoiceBulkActionForm(forms.Form):
    ...
    invoice_ids = forms.CharField(widget=forms.HiddenInput(), required=False)
