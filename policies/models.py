...
class Policy(models.Model):
    ...
    # Policy fields
    policy_number = models.CharField(_("Número de póliza"), max_length=50, unique=True)
    ...
    group_type = models.CharField(
        _("Tipo de grupo"), max_length=20, choices=GROUP_TYPE_CHOICES
    )
    subgroup = models.CharField(_("Subgrupo"), max_length=100)
    branch = models.CharField(_("Rama"), max_length=100)
    ...
    # Additional info
    coverage_description = models.TextField(_("Descripción de cobertura"), blank=True)
    ...
    def calcular_valores_fiscales(self):
        ...
        self.contrib_superintendencia = (self.prima * Decimal("0.035")).quantize(
            Decimal("0.01")
        )

        # 2. Contribución Seguro Campesino (0.5% de prima)
        self.contrib_seguro_campesino = (self.prima * Decimal("0.005")).quantize(
            Decimal("0.01")
        )
    ...
    def generate_policy_number():
        ...
        last_policy = (
            Policy.objects.filter(policy_number__startswith=f"POL-{current_year}-")
...
class PolicyDocument(models.Model):
    ...
    # File fields
    file = models.FileField(_("Archivo"), upload_to="policies/documents/%Y/%m/")
    ...
    def get_document_types_for_policy(policy):
        ...
        return [
            (doc_type, dict(PolicyDocument.DOCUMENT_TYPE_CHOICES)[doc_type])
...
class Coverage(models.Model):
    ...
    is_active = models.BooleanField(
        _("Activa"), default=True, help_text=_("Si la cobertura está activa")
    )
    ...
    def calcular_deducible(self, valor_siniestro):
        ...
        deducible_porcentual = valor_siniestro * (
            self.porcentaje_deducible / Decimal("100")
        )
    ...
    def clean(self):
        ...
        if self.porcentaje_deducible < 0 or self.porcentaje_deducible > 100:
            raise ValidationError(
                _("El porcentaje de deducible debe estar entre 0 y 100")
            )
