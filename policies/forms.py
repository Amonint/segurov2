...
class PolicyForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["responsible_user"].queryset = UserProfile.objects.filter(
            is_active=True, role__in=["admin", "insurance_manager"]
        )
    ...
    def clean(self):
        ...
        if issue_date and start_date and issue_date > start_date:
            raise forms.ValidationError(
                _("La fecha de emisión no puede ser posterior a la fecha de inicio")
            )
...
class PolicySearchForm(forms.Form):
    ...
    search = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"placeholder": _("Número de póliza, compañía...")}
        ),
    )
...
class PolicyRenewalForm(forms.ModelForm):
    ...
    def clean_new_end_date(self):
        ...
        if self.current_policy and new_end_date <= self.current_policy.end_date:
            raise forms.ValidationError(
                _("La nueva fecha de fin debe ser posterior a la fecha actual")
            )
        return new_end_date
    ...
    class Meta:
        ...
        widgets = {
            "document_name": forms.TextInput(
                attrs={"placeholder": _("Nombre descriptivo del documento")}
            ),
            "description": forms.Textarea(
                attrs={
                    "rows": 2,
                    "placeholder": _("Descripción opcional del documento"),
                }
            ),
            "tags": forms.TextInput(
                attrs={"placeholder": _("Etiquetas separadas por comas")}
            ),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.policy:
            self.fields["document_type"].choices = (
                PolicyDocument.get_document_types_for_policy(self.policy)
            )
    ...
    def clean_file(self):
        ...
        if file:
            ...
            if hasattr(file, "content_type") and file.content_type not in allowed_types:
                raise forms.ValidationError(
                    _(
                        "Tipo de archivo no permitido. Solo se permiten PDFs, documentos Word, Excel, imágenes y archivos de texto."
                    )
                )
...
class PolicyDocumentEditForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.policy:
            self.fields["document_type"].choices = (
                PolicyDocument.get_document_types_for_policy(self.instance.policy)
            )
