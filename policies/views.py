...
def policy_list(request):
    ...
    policies = (
        Policy.objects.all()
        .select_related("insurance_company", "broker", "responsible_user")
        .order_by("-created_at")
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            old_values = {
                "policy_number": policy.policy_number,
                "insurance_company": (
                    policy.insurance_company.name if policy.insurance_company else None
                ),
                ...
                "status": policy.status,
                "responsible_user": (
                    policy.responsible_user.username
            ...
            new_values = {
                ...
                "insurance_company": (
                    updated_policy.insurance_company.name
                    if updated_policy.insurance_company
                    else None
                ),
                ...
                "responsible_user": (
                    updated_policy.responsible_user.username
                    if updated_policy.responsible_user
                    else None
                ),
            }
...
def policy_delete(request, pk):
    ...
    if has_claims or has_invoices or has_assets:
        messages.error(
            request,
            _(
                "No se puede eliminar la póliza porque tiene registros asociados (siniestros, facturas o bienes asegurados)."
            ),
        )
        ...
        if form.is_valid():
            ...
            try:
                ...
                AuditLog.log_action(
                    ...
                    new_values={
                        "status": "renewed",
                        "new_policy_number": renewed_policy.policy_number,
                    },
                )
...
def policy_document_upload(request, pk):
    ...
    if request.method == "POST":
        form = PolicyDocumentForm(
            request.POST, request.FILES, policy=policy, uploaded_by=request.user
        )
        if form.is_valid():
            document = form.save()
    ...
    return render(
        request,
        "policies/policy_document_form.html",
        {
            "form": form,
            "policy": policy,
...
def policy_document_new_version(request, document_pk):
    ...
    document = get_object_or_404(
        PolicyDocument.objects.filter(is_latest_version=True), pk=document_pk
    )
    policy = document.policy
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new_version.document_name = form.cleaned_data.get(
                "document_name", document.document_name
            )
            new_version.document_type = form.cleaned_data.get(
                "document_type", document.document_type
            )
            ...
            messages.success(
                request, _("Nueva versión del documento subida exitosamente.")
            )
            return redirect(
                "policies:policy_document_versions", document_pk=document.pk
            )
    else:
        ...
        form = PolicyDocumentForm(
            policy=policy, uploaded_by=request.user, initial=initial_data
        )
...
def policy_document_versions(request, document_pk):
    ...
    return render(
        request,
        "policies/policy_document_versions.html",
        {
            "policy": policy,
            "document": document,
            "versions": versions,
...
def policy_document_view(request, document_pk):
    ...
    return render(
        request,
        "policies/policy_document_view.html",
        {
            "policy": policy,
            "document": document,
        },
...
def policy_bulk_action(request):
    ...
    if action == "delete":
        ...
        for policy in policies:
            # Check if policy has dependencies
            if not (
                policy.claims.exists()
                or policy.invoices.exists()
                or policy.insured_assets.exists()
            ):
                policy.delete()
                deleted_count += 1
        ...
        if deleted_count > 0:
            messages.success(
                request, f"{deleted_count} pólizas eliminadas exitosamente."
            )
        else:
            messages.warning(
                request,
                _(
                    "No se pudieron eliminar las pólizas seleccionadas porque tienen registros asociados."
                ),
            )
