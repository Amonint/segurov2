...
def broker_list(request):
    ...
    if search_form.is_valid():
        ...
        if commission_range:
            if commission_range == "low":
                ...
                brokers = brokers.filter(
                    commission_percentage__gte=5, commission_percentage__lt=15
                )
    ...
    avg_commission = (
        brokers.filter(is_active=True).aggregate(avg=Avg("commission_percentage"))[
            "avg"
        ]
        or 0
    )
...
def broker_detail(request, pk):
    """View broker details"""
    broker = get_object_or_404(Broker.objects.prefetch_related("policies"), pk=pk)
...
def broker_delete(request, pk):
    ...
    if broker.policies.exists():
        messages.error(
            request,
            _("No se puede eliminar el corredor porque tiene pólizas asociadas."),
        )
...
def broker_toggle_active(request, pk):
    ...
    messages.success(
        request, _("Corredor %(action)s exitosamente.") % {"action": action}
    )
    return redirect("brokers:broker_detail", pk=pk)
