...
class Asset(models.Model):
    ...
    # Asset fields
    asset_code = models.CharField(_("Código del activo"), max_length=50, unique=True)
    name = models.CharField(_("Nombre del bien"), max_length=255)
    ...
    asset_type = models.CharField(
        _("Tipo de activo"), max_length=20, choices=ASSET_TYPE_CHOICES
    )
    location = models.CharField(_("Ubicación física"), max_length=255)
    ...
    # Insurance
    is_insured = models.BooleanField(_("¿Está asegurado?"), default=False)
    ...
    def calculate_depreciation(self, years=None):
        ...
        depreciated_value = self.acquisition_cost * (
            1 - depreciation_rate * Decimal(str(min(years, 10)))
        )
    ...
    def generate_asset_code():
        ...
        last_asset = (
            Asset.objects.filter(asset_code__startswith=f"ACT-{current_year}-")
            .order_by("-asset_code")
            .first()
        )
