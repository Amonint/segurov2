...
class AssetClientForm(forms.ModelForm):
    """
    Simplified form for clients (requesters) to create their own assets.
    Clients provide basic information and the system auto-assigns them as custodian.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["acquisition_date"].required = True
        self.fields["acquisition_cost"].required = True
        self.fields["current_value"].required = True

        # Filter insurance policies - show only active policies available to the user
        if self.user:
            self.fields["insurance_policy"].queryset = Policy.objects.filter(
                status="active"
            )
            # Help text
            self.fields["is_insured"].help_text = _(
                "Si marca esta opción, debe seleccionar una póliza de seguro"
            )
            self.fields["insurance_policy"].help_text = _(
                "Seleccione una póliza de seguro (solo si el bien está asegurado)"
            )
